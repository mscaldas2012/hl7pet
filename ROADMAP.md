...
| `008` | `->` child-side numeric index (e.g. `OBX[2]`) is type-filtered, re-based per matching parent occurrence, and 1-based, instead of reproducing the real Scala engine's documented bug (spec `002` Section A.4: cross-parent concatenation, cross-type mixing, no 1-based adjustment) | An `OBX[N]` child index can select a different (correct) segment than the real engine's untested, buggy behavior would for the same message |
...
| 008    | Rust Core | `lazy-hierarchy-nav` | Implemented — hierarchy executor (`crates/core/src/hierarchy.rs`, new sibling module to specs `005`-`007`) resolving `CompiledPath.child` (`->`) against a `ScanResult` and a new `HierarchyProfile` (parsed from `segmentDefinition` JSON via `serde`/`serde_json`, promoted from a dev- to a runtime dependency — the crate's first, pure-Rust with no system/C-library step, kept out of the public API per FR-014). Navigation is a bounded, per-parent-occurrence forward scan seeded at the parent's own profile node — never a materialized full-message tree (spec `002` Section B.1). A significant planning-time design correction found only during implementation, verifying against the real fixture profiles rather than an assumption: `deep-nested.json` legitimately places `OBX` (and `NTE`) as a legal child at *two different* nesting depths (directly under `OBR`, and again under `OBR`'s `SPM` child) — a normal, common profile shape, not malformed data. An earlier design rejected any such repeated segment-type placement outright at profile-construction time; verifying against the actual fixture data (not just a hand-picked example) showed this would incorrectly reject the very profile 5 of 6 `complex.json` vectors already depend on. Corrected: only the *parent*-side type of a `->` expression needs unambiguous resolution (to seed the local scan); descendant matching is already correctly disambiguated by walking each node's own children map regardless of how many other places that type appears elsewhere in the tree — `ProfileError` ended up with a single variant, `InvalidJson` (malformed JSON only; a true structural cycle is unrepresentable in `segmentDefinition`'s JSON-tree shape, confirmed against the real Scala source, `HL7HierarchyParser.scala`). Single-hop only, as decided in this spec's Clarifications: spec `002` Section B.2's recommended multi-hop `CHILD_PATH` chaining is deferred to a future spec — confirmed spec `006`'s parser still rejects a second `" -> "` (`MultipleHierarchyHops`) unchanged, and 2 of the corpus's 10 pre-existing vectors (`hier-009`, `hier-010`) use exactly such a two-hop PATH, discovered during implementation to be unexecutable under this spec's scope and excluded from its test harness with the exclusion documented, rather than silently miscounted as passing. The documented child-index bug (spec `002` Section A.4: cross-parent concatenation, cross-type mixing, no 1-based adjustment) is fixed rather than reproduced — a documented Breaking Change (see table above); this required correcting `hier-004`/`hier-008`'s `expected` values to the fixed (not buggy) output, each independently verified by hand against `HL7ParseUtils.scala` before editing. A new vector, `hier-011`, was added (with `messages/basic-hierarchy.hl7` extended to carry a second `OBR` and its own `OBX` children) specifically to prove two `OBR` occurrences' children stay isolated from each other — the pre-existing corpus had no vector demonstrating this directly. 9/9 executable `fixtures/vectors/hierarchy/` vectors pass (11 total minus the 2 excluded multi-hop vectors); full corpus validation passes (46/46 vectors, 10/10 hierarchy semantic rules); 9 new unit tests plus the `hierarchy_vectors` integration test pass alongside the full pre-existing suite (51 unit tests crate-wide); `cargo clippy --workspace --all-targets` clean; all 8 `quickstart.md` steps pass, including the `hl7pet` dev CLI's new `--profile` flag. Ready for spec `009` (core-perf-validation) to benchmark this module, and for a future spec to revisit multi-hop chaining. |
| 009    | Rust Core | `core-perf-validation` | Implemented — extended spec `004`'s Maven/JMH harness (new `HierarchyBenchmarks.java`, covering hierarchy mode for the first time) and built a new `cargo bench` harness (`crates/core/benches/`, hand-written `Instant`-sampling, no `criterion`) benchmarking `scan`/`execute`/`execute_hierarchy` against `HL7StaticParser`/`HL7ParseUtils`. Completed spec `004`'s own deferred corpus migration: its 27-message interim corpus relocated to the shared `fixtures/messages/perf/` (`corpusId: perf-v2`), plus one new large synthetic hierarchy message (20 `OBR` × 5 `OBX`) neither prior corpus had. **Real verdict (2026-09-04 run, `forks(3)`, full production JMH config): zero regressions.** Rust beats Scala on every one of 92 comparable metrics — throughput and latency 2-2600x, allocated bytes/op 1.2-449x; the extreme end is entirely hierarchy navigation, where Scala's `HL7ParseUtils` rebuilds a full parse tree from scratch on every call (spec 002 Section A.1/B.1's documented eager-tree design) while Rust's bounded scan only touches what a query actually requests — the first real, at-scale confirmation of the exact architectural bet specs 002/008 made. Allocation counts stay small and constant per feature regardless of message size (`parsing`: always 2, confirming spec 005's own claim at corpus scale) and scale only with result count, never with an accidentally-unbounded full-message pass — confirmed directly for hierarchy (18 → 37 → 611 allocations tracking 1 → 5 → 100 matched occurrences, not the message's 123 lines). One real methodology bug found and fixed against the actual data: `memoryAllocRateBytesPerSec` (bytes/op × throughput) initially reported 21 false "regressions" — an engine completing dramatically more operations per second necessarily moves more total bytes per second too, even while using far less memory per call; every flagged row's own `throughput`/`allocationBytesPerOp` showed Rust winning 10-20x on the same data. Fixed: that metric's verdict is now `"not-comparable"` (values kept for reference); `allocationBytesPerOp` is the metric that actually answers the memory question. Full artifacts and results breakdown at `specs/009-core-perf-validation/comparison/2026-09-04/` and that spec's `tasks.md`. |
| 1000   | Parsing & Extraction | `located-extraction-api` | Implemented — two new query-execution entry points in `crates/core/src/query.rs` (spec `007`'s home, no new module): `execute_located` (location-aware counterpart to `execute`/`getValue`) and `first_located` (counterpart to `getFirstValue`, a thin wrapper — there was no dedicated `execute_first` in the core to parallel), both additive alongside the unchanged `execute()`. A new `LocatedValue<'m> { value: &'m str, line: usize }` pairs every value with the 1-based position of its source segment occurrence among all segments in the message, derived from `ScanResult.segments`'s existing document order — no new scan. Real finding while implementing: `resolve_segment_candidates` (spec `007`, already called directly by `hierarchy.rs` and its own unit tests) could not have its return type changed in place without rippling into spec `008`'s hierarchy module; fixed by factoring its filtering logic into a new `resolve_segment_candidates_indexed` that it now delegates to, leaving every existing call site (`execute()`, `hierarchy::execute_hierarchy`, five pre-existing unit tests) byte-for-byte unchanged — confirmed by the full pre-existing suite passing with zero modifications. A second finding, caught by a counting-allocator test: `execute_located` costs one small, fixed allocation more than `execute()` per matched occurrence (a `&str -> LocatedValue` map/collect is a different element size than `execute()`'s `&str -> &str` one, so it can't reuse the standard library's same-size in-place collect optimization) — a constant, not a scaling cost, so SC-004's actual requirement (allocation count independent of message size) still holds and is what the test verifies. All 15 `expected_lines`-bearing vectors in the shared fixtures corpus (`fixtures/vectors/path/valid.json`, spec `001` FR-008) pass via `execute_located`, covering both single- and multi-occurrence/filtered cases; no new vectors or schema needed. `hl7pet` dev CLI gained a `--located` flag (mirroring `--first`/`--profile`), rejecting hierarchy PATHs cleanly per this feature's explicit non-hierarchy scope. This is a new capability with no Scala equivalent, so no comparative baseline benchmark applies (unlike specs `004`/`009`). 71 tests pass workspace-wide (30 `hl7pet-core` unit tests including 12 new, plus all integration test files); `cargo clippy --workspace --all-targets` clean. Full detail at `specs/1000-located-extraction-api/research.md` (5 documented decisions) and that spec's `tasks.md`. |
